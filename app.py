    ...
    try:
        for fname in os.listdir(icons_dir):
            ...
            if ext.lower() in ICON_EXTS:
                ...
                if base not in mapping or (
                    os.path.splitext(mapping[base])[1].lower() != ".png" and ext.lower() == ".png"
                ):
                    mapping[base] = fname
    except Exception:
        pass
...
def print_tasks(subtitle_tasks):
    ...
    try:
        ...
        if config["printer_type"] == "usb":
            p = (
                Usb(int(config["usb_vendor_id"], 16), int(config["usb_product_id"], 16), 0, profile=profile)
                if profile
                else Usb(int(config["usb_vendor_id"], 16), int(config["usb_product_id"], 16))
            )
        elif config["printer_type"] == "network":
            from escpos.printer import Network

            p = (
                Network(config["network_ip"], int(config["network_port"]), profile=profile)
                if profile
                else Network(config["network_ip"], int(config["network_port"]))
            )
        elif config["printer_type"] == "serial":
            from escpos.printer import Serial

            p = (
                Serial(config["serial_port"], baudrate=int(config["serial_baudrate"]), profile=profile)
                if profile
                else Serial(config["serial_port"], baudrate=int(config["serial_baudrate"]))
            )
        else:
        ...
        app.logger.exception(f"Printer error: {e!s}")
        return False
...
def _do_test_print(config_override=None):
    ...
    test_pairs = [
        ...
        (datetime.now().strftime("%Y-%m-%d %H:%M"), "Hello from Task Printer!"),
    ]
...
def healthz():
    status = {
        ...
        "worker_alive": bool(globals().get("PRINT_WORKER_THREAD", None))
        and globals()["PRINT_WORKER_THREAD"].is_alive(),
